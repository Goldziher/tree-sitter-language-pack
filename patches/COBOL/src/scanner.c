...
// MSVC doesn't support VLAs. Use fixed-size arrays with a reasonable maximum.
// The current usage has at most 9 keywords, so 64 is a safe upper bound.
...
static bool start_with_word( TSLexer *lexer, char *words[], int number_of_words) {
    ...
    while(true) {
        ...
        for(int i=0; i<number_of_words; ++i) {
            ...
            if(continue_check[i]) {
                continue_check[i] = (unsigned char)c == towupper((unsigned char)k) || (unsigned char)c == towlower((unsigned char)k);
            }
            (keyword_pointer[i])++;
        }
